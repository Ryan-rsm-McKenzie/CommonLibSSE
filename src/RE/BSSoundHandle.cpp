...
namespace RE
{
	...
	{
		using func_t = function_type_t<decltype(&BSSoundHandle::IsValid)>;
	...
	{
		using func_t = function_type_t<decltype(&BSSoundHandle::SetObjectToFollow)>;
	...
	{
		using func_t = function_type_t<decltype(&BSSoundHandle::Stop)>;
