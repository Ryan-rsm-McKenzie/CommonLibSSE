...
namespace RE
{
	...
	public:
		...
		struct FriendshipFactions
		{
			UInt16 friendCounts[4];	// 0
		};
		STATIC_ASSERT(sizeof(FriendshipFactions) == 0x8);
		...
		STATIC_ASSERT(sizeof(TargetLoc) == 0x48);
		...
		UInt32												unk444;							// 444
		BSTHashMap<const TESFaction*, FriendshipFactions>	factionOwnerFriendsMap;			// 448
