...
namespace RE
{
	...
	public:
		...
		enum class TranslateCaps : UInt32
		{
			kNone = 0,
			...
			kBidirectionalText = 1 << 2			// Enables support for bidirectional text. Method OnBidirectionalText will be called
		};


		// An enumeration that describes bits in word-wrapping mode bit mask parameter of the Translator constructor.
		enum class WordWrappingTypes : UInt32
		{
			kDefault = 0,											// - Custom word-wrapping is off, OnWordWrapping will not be invoked
			//kAsian = Text::WordWrapHelper::kAsian,				// - Turns on Chinese word-wrapping rules
			//kProhibition = Text::WordWrapHelper::kProhibition,	// - Prohibits certain chars at start/end of line (turns on Japanese prohibition rule)
			//kNoHangulWrap = Text::WordWrapHelper::kNoHangulWrap,	// - Turns on Korean-specific word-wrapping rules
			//kHyphenation = (Text::WordWrapHelper::kLast << 1),	// - Turns on a simple hyphenation; for demo purposes only
			kCustom = 0x80,											// - Turns on custom user-defined word-wrapping. User should implement his own version of OnWordWrapping virtual method
		...
		{
		public:
			static constexpr UPInt MAX_LEN = std::numeric_limits<UPInt>::max();
			...
			void			SetResult(const wchar_t* a_resultText, UPInt a_resultLen = MAX_LEN);		// An output method which sets the translated string as a plain text.
			void			SetResult(const char* a_resultTextUTF8, UPInt a_resultUTF8Len = MAX_LEN);	// An output method which sets the translated string as a plain text.
			void			SetResultHtml(const wchar_t* a_resultHtml, UPInt a_resultLen = MAX_LEN);	// An output method which sets translated string as a HTML text.
			void			SetResultHtml(const char* a_resultHtmlUTF8, UPInt a_resultLen = MAX_LEN);	// An output method which sets translated string as a HTML text.
		...
		constexpr GFxTranslator() :
			wwMode(WordWrappingTypes::kDefault),
			pad1C(0)
		{}

		explicit constexpr GFxTranslator(WordWrappingTypes a_wwMode) :
			wwMode(a_wwMode),
			pad1C(0)
		{}
		...
		// add
		virtual TranslateCaps	GetCaps() const;							// 01 - { return TranslateCaps::kNone; } - Specifies capabilities of the Translate implementation
