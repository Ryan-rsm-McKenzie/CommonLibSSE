...
namespace RE
{
	...
	public:
		...
		virtual void			SetBackgroundColor(const void* a_bgColor);																		// 2A - pure
		virtual void			SetBackgroundAlpha(float a_alpha);																				// 2B - pure
		virtual float			GetBackgroundAlpha() const;																						// 2C - pure
		virtual UInt32			HandleEvent(const void* a_event);																				// 2D - pure
		...
		virtual bool			HitTest3D(void* a_ptOut, float a_x, float a_y, UInt32 a_controllerIdx = 0);										// 31 - pure
		...
		virtual void			GetStats(void* a_bag, bool a_reset = true);																		// 3C - pure
		virtual void*			GetHeap() const;																								// 3D - pure
		virtual void			ForceCollectGarbage();																							// 3E - pure
		virtual void*			TranslateToScreen(const void*& a_p, void* a_userMatrix);														// 3F - pure
		virtual GRectF			TranslateToScreen(const GRectF& a_p, void* a_userMatrix);														// 40 - pure
		virtual bool			TranslateLocalToScreen(const char* a_pathToCharacter, const void* a_pt, void* a_presPt, void* a_userMatrix);	// 41 - pure
