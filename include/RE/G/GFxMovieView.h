...
namespace RE
{
	...
	public:
		...
		virtual std::uint32_t				GetMouseCursorCount() const;																						// 39 - pure
		virtual void						SetControllerCount(std::uint32_t a_count);																			// 3A - pure
		virtual std::uint32_t				GetControllerCount() const;																							// 3B - pure
		...
		virtual std::uint32_t				GetControllerFocusGroup(std::uint32_t a_controllerIdx) const;														// 43 - pure
